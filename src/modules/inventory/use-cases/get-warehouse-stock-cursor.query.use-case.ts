...
export type GetWarehouseStockCursorQuery = {
  current_user: AuthenticatedUserContext;
  query: CursorQueryDto;
};

@Injectable()
export class GetWarehouseStockCursorQueryUseCase
  implements
    QueryUseCase<
      GetWarehouseStockCursorQuery,
    ...
    >
{
  ...
  async execute({
    current_user,
    query,
  }: GetWarehouseStockCursorQuery): Promise<CursorResponseDto<WarehouseStockView>> {
    const business_id = resolve_effective_business_id(current_user);
    const accessible_branch_ids =
      this.inventory_validation_service.resolve_accessible_branch_ids(
        current_user,
      );
    const page = await this.inventory_balances_repository.find_cursor_by_business(
      business_id,
