...
export function serialize_terminal(terminal: Terminal) {
  return {
    id: terminal.id,
    code: terminal.code,
...
export function serialize_branch(branch: Branch) {
  return {
    id: branch.id,
    code: branch.code,
    ...
    terminals:
      branch.terminals?.map((terminal) => serialize_terminal(terminal)) ?? [],
  };
}
